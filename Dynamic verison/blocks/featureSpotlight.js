  ...
  render: (p, { mergeSurfaceStyle, html, attr }) => `
    <div class="block" data-type="featureSpotlight">
      <div data-surface="1" style="position:relative; display:flex; flex-direction:column; align-items:center; ${mergeSurfaceStyle(p)}">
        <div style="position:absolute; top:-20px; font-size:120px; font-weight:900; opacity:0.03; color:${attr(p.textColor)}; letter-spacing:20px; pointer-events:none; white-space:nowrap;">${html(p.watermark)}</div>
        <div style="max-width:800px; text-align:center; z-index:1;">
          <h2 style="font-size:48px; font-weight:800; color:${attr(p.textColor)}; margin:0 0 40px 0;">${html(p.title)}</h2>
          ...
          <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:20px;">
            ${p.bullets.map(b=>`
              <div style="background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); padding:12px 24px; border-radius:99px; color:${attr(p.textColor)}; font-size:14px; font-weight:700; display:flex; align-items:center; gap:10px;">
