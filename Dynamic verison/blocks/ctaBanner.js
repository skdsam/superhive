export default {
  ...
  render: (p, { mergeSurfaceStyle, html, attr }) => `
    <div class="block" data-type="ctaBanner">
      <div data-surface="1" style="${mergeSurfaceStyle(p)} ${p.bg?`background:${p.bg};`:``} ${p.color?`color:${p.color};`:``}">
        <h2 style="font-size:32px; font-weight:900; margin:0 0 12px 0;">${html(p.headline)}</h2>
        <p style="font-size:18px; color:rgba(255,255,255,0.7); margin:0 0 32px 0;">${html(p.sub)}</p>
